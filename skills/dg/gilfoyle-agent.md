...
| `build.gradle` | Gradle | See Gradle note below |
| `Gemfile.lock` | Ruby | `bundle audit check` |
| `Cargo.lock` | Rust | `cargo audit` |
| `composer.lock` | PHP | `composer audit --format=json` |

_Gradle note_: Before invoking `./gradlew dependencyCheckAnalyze`, check whether
the OWASP dependency-check plugin is actually configured:

```bash
grep -r "dependency-check" build.gradle settings.gradle build.gradle.kts 2>/dev/null || \
grep -r "org.owasp" build.gradle settings.gradle build.gradle.kts 2>/dev/null
```

If the plugin is absent, **do not run `./gradlew dependencyCheckAnalyze`** — skip directly
to the OSV.dev API fallback. Running Gradle tasks on unconfigured plugins triggers full
build resolution, which produces unwanted compilation artifacts (`bin/`, `build/`) and
may regenerate IDE project files (`.project`, `.classpath`).

If the plugin IS present, run `./gradlew dependencyCheckAnalyze --no-daemon` to avoid
spawning a persistent daemon that holds file locks.
