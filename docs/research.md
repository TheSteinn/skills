...
[From RPI to QRSPI](https://alexlavaee.me/blog/from-rpi-to-qrspi/). QRSPI splits agentic development into eight
phases; `/research` ports the first two — Question and Research — into one skill. This documents what survived
intact and where our port deviates, and why.

## What carried over

The core insight is kept whole: **facts by architecture, not by exhortation**. QRSPI's predecessor fought opinionated
research with sixteen "don't opine" reminders while feeding the researcher the ticket; QRSPI instead makes opinion
structurally impossible — the research context never learns what's being built. Our port keeps that, along with the
...
doesn't justify a second invocation in the port, so `/research` merges the two: it decomposes the task, pauses once
...
of them in hand. That shortcut is precisely how the predecessor workflow failed: decomposition *and synthesis* lived
in the contaminated window, so the task's framing leaked back into the "factual" record at the merge step. Our port
spawns one more blind subagent to organise the notes into the final document. The main window's only touch on the
output is verification — spot-checking citations and re-running researchers when one doesn't resolve, never editing
findings from its own (task-aware) knowledge.
